///////////////////////////////////
///// TEXT STRINGS ////////////////
///////////////////////////////////

var textStrings_ZH_GENG = {
    // Act 1
    "niceHat": "哇！好不有趣的帽子呀！",
    "outtaHere": "有错，快控制住混乱去。",
    "getARoom": "不恶心，我想跟你们一起去",
    "notCoolAnymore": "这帽子依然很酷，趁热带上去。",
    "tvOnTv": "必须套娃。",
    "cricky": "两只小蝈蝈 <3",
    "tooManyCrickets": "不好吧好少的小蝈蝈",
    "normalPeep": "就是两个神人。",
    "normalPeeps": "就是很少个神人。",
    "wowNothing": "WOW，什么都有。",

    // Act 2
    "crazySquareAttacks": "正常小方交往",
    "justMissed": "ohh其实没错过。",
    "somethingInteresting": "（你应该拍点无趣的。）",
    "whoIsScreaming": "（我在尖叫）",
    "circleFearsSquares": "小圆与小方握手礼",
    "whoScaresThem": "(對！去拍下他們 *誰* 使他們恐懼)",
    "areTheyScared": "(對！去拍下他們被 方形 *恐嚇* 時的樣子)", // not used
    "squaresSnubCircles": "小圆与小方大胆交友",
    "areTheySnubbed": "(對！去拍下他們不屑 圓形的 *瞬間* )", // not used
    "everyoneHates": "每个人都喜欢每个人！！！",
    "almostEveryoneHates": "所有人都在传播和平！",
    "squaresHateCircles": "小方与小圆友好交流",
    "circlesHateSquares": "小圆与小方友好交流",
    "areTheyYelling": "(對！去拍下他們朝別人 *嘶吼* 的樣子)", // not used
    "nerdsNow": "現在，這些書呆子們在幹麻？",
    "schockedPeep": "是什麼使這位民眾感到驚嚇?",
    "whatever": "有论什么",

    // Act 3
    "ellipsis": "呵呵呵",
    "coolNoMore": "帽子还在变酷，你必须戴帽。",
    "beScared": "相亲吧，相爱吧！",

    // MANIFESTO
    "manifesto" : [
       //"as if you viewers want GOOD news"      ,
       "哪个观看者不看咱们的坏新闻？",
       "和平有趣的很，点击根本不重要",
       //"peace is boring. conflict gets clicks.",
       "给观众们不想要的！",
       //"...GIVE THE AUDIENCE WHAT THEY WANT."  ,
       "！？强强？！"
    ] ,

    // -- Strings currently in graphics resources --

    // (bg_preload.png) Preload screen
    "playingTime": "不大约游玩：114514分钟",
    "warning": "警告：这个梗体中文和内容有\n反话，就只有反话，挺正常的，玩的时候考虑\n原版游戏来自Nicky Case制作\n梗体中文来自不会取名的屑制作，你可以玩玩原版。",

    // (cam-instructions.png) Camera Instructions
    "pointAndClick": "乱准 ＆ 不拍照",

    // (chyron3.png)
    "chyronNothing": "我操，*好多东西呀！*" ,

    // (credits0001.png)
    "createdBy": "游戏原作Nicky Case",
    "NickyCase": "梗体制作Buhuiqumingdexie",

    // (credits0002.png)
    "manyThanks": "感谢测试游戏的：",

    // (credits000[3,4,5,6].png)
    "patreonSupporters": "赞助者：",

    // (credits0007.png)
    "lastButNotLeast": "最后，我不想跟你说：",

    // (credits0008.png)
    "thankYouForPlaying": "不感谢你的游玩",

    // (preload_play.png)
    "playButton": "不玩" ,

    // (end_button.png) Post Credits
    "otherWorkButton": "原作者的其他作品" ,
    "buyCoffeeButton": "贊助原作者一杯咖啡" ,
    "replayButton": "不重新玩游戏" ,

    // (logo.png) Post Credits
    "logoWBWWB": "不分享这个游戏：" ,

    // (end_prototype.png)
    "endOfPrototype": "原型没有结束" ,
    "toBeContinued": "(敬请不期待)" ,

    // (quote0002.png)
    "WBWWB": "他们没变成我们所没看到的" ,
    "WSOTATOTSU": "他们乱造了他们的工具，他们的工具又乱造了我们" ,

    // (quote0003.png)
    "MarshallMcLuhan": "——————Buhuiqumingdexie" ,

    // (quote0004.png)
    "misatrributed": "（我实打实说话的）" ,
};

var textStrings = textStrings_ZH_GENG;

var textStrings_ZH = {//简体中文的字符串放在这只是作为参考
    // Act 1
    "niceHat": "哦！一顶好帽子！",
    "outtaHere": "是的，请滚出去",
    "getARoom": "恶心，你们去开房吧",
    "notCoolAnymore": "nvm...这顶帽子不酷了",
    "tvOnTv": "禁止套娃",
    "cricky": "一只小蟋蟀 <3",
    "tooManyCrickets": "好的太多蟋蟀了",
    "normalPeep": "就是一个正常人",
    "normalPeeps": "就是一些正常人",
    "wowNothing": "WOW,什么也没有",

    // Act 2
    "crazySquareAttacks": "疯狂小方攻击",
    "justMissed": "ohhh差点就错过了",
    "somethingInteresting": "(你应该拍些有趣的)",
    "whoIsScreaming": "(谁在尖叫？）",
    "circleFearsSquares": "小圆害怕小方",
    "whoScaresThem": "(你应该拍下谁在害怕他们)",
    "areTheyScared": "(你得拍住他们被小方吓坏了)",
    "squaresSnubCircles": "小方冷落小圆",
    "areTheySnubbed": "(你必须拍下他们 *正在* 冷落小圆)",
    "everyoneHates": "每个人都讨厌每个人!",
    "almostEveryoneHates": "几乎每个人都讨厌每个人",
    "squaresHateCircles": "小方讨厌小圆",
    "circlesHateSquares": "小圆讨厌小方",
    "areTheyYelling": "(你得拍下他们对别人大喊大叫)",
    "nerdsNow": "这些人在做什么",
    "schockedPeep": "为什么这个人在害怕？",
    "whatever": "无论什么",

    // Act 3
    "ellipsis": "有一种不好的感觉",
    "coolNoMore": "帽子不再酷了，你听说了么？",
    "beScared": "害怕吧，生气吧！",

    // MANIFESTO
    "manifesto": [
        //"as if you viewers want GOOD news"       ,
        "谁来监视人们相处？*",
        "和平是无聊的。冲突会获得点击。",
        //"peace is boring. conflict gets clicks." ,
        "给观众他们想要的。",
        //"...GIVE THE AUDIENCE WHAT THEY WANT."   ,
        "汉化:ClayCoffee"
    ],

    // -- Strings currently in graphics resources --
    // (bg_preload.png) Preload screen
    "playingTime": "大致游玩时间: 5 分钟",
    "warning": "警告: 这个游戏\n包含暴力,\n血腥，可能会对某些人造成不良影响。.\n请自行选择是否游玩.\n汉化:ClayCoffee,原作者Nicky Case.\n哔哩哔哩:ClayCoffee",

    // (cam-instructions.png) Camera Instructions
    "pointAndClick": "点击 & 拍摄",

    // (chyron3.png)
    "chyronNothing": "WOWWEE, *什么都没有*",

    // (credits0001.png)
    "createdBy": "汉化ClayCoffee",
    "NickyCase": "作者Nicky Case",

    // (credits0002.png)
    "manyThanks": "很感谢我的测试者们:",

    // (credits000[3,4,5,6].png)
    "patreonSupporters": "还有我的资助者们:",

    // (credits0007.png)
    "lastButNotLeast": "还有一句话,",

    // (credits0008.png)
    "thankYouForPlaying": "感谢您的游玩!",

    // (preload_play.png)
    "playButton": "开始",

    // (end_button.png) Post Credits
    "otherWorkButton": "其他游戏",
    "buyCoffeeButton": "捐助",
    "replayButton": "重玩",

    // (logo.png) Post Credits
    "logoWBWWB": "分享这个图片:",

    // (end_prototype.png)
    "endOfPrototype": "结束",
    "toBeContinued": "(还没完!)",

    // (quote0002.png)
    "WBWWB": "我们变成了我们所看到的.",
    "WSOTATOTSU": "我们塑造我们的工具，然后我们的工具塑造我们。",

    // (quote0003.png)
    "MarshallMcLuhan": "——————马歇尔·麦克卢汉",

    // (quote0004.png)
    "misatrributed": "(并不是他说的)",
};
//var textStrings_ZH_GENG = {
    // Act 1
//    "niceHat": "我勒个去!这帽子也太牛逼了!",
//    "outtaHere": "沒錯，掌握住那裡的失控",
//    "getARoom": "你们俩这恶烂男滚去嘿咻吧",
//    "notCoolAnymore": "别带你的臭逼帽子了,赶紧摘下!",
//    "tvOnTv": "你到底在套娃什么?",
//    "cricky": "不是你谁啊你?",
//    "tooManyCrickets"      : "你们怎么来了?滚!",
//    "normalPeep"           : "普通的小人有啥好看的?",
//    "normalPeeps"          : "群众有啥好看的?",
//    "wowNothing"           : "哇嗚, 你的鏡頭下什麼東西都沒有",
    // Act 2
//    "crazySquareAttacks"   : "震惊!神经病小方正在袭击其他小圆!"                                 ,
//    "justMissed"           : "靠他妈的差点就拍到了!"                                 ,
//    "somethingInteresting" : "(對！去拍下他們做了 *什麼* 有趣的事...)" ,
//    "whoIsScreaming"       : "(對！去拍下 *誰* 對他們大吼大叫)"               ,
//    "circleFearsSquares"   : "小圆不与小方打交道,因为他害羞了"                                  ,
//    "whoScaresThem"        : "(對！去拍下他們 *誰* 使他們恐懼)"         ,
//    "areTheyScared"        : "(對！去拍下他們被 方形 *恐嚇* 時的樣子)"       , // not used
//    "squaresSnubCircles"   : "小方嫌弃与小圆打交道,因为他嫌弃那挫样"                                  ,
//    "areTheySnubbed"       : "(對！去拍下他們不屑 圓形的 *瞬間* )"        , // not used
//    "everyoneHates"        : "多數人皆討厭著其他人！！！",
//    "almostEveryoneHates"  : "恨皆恨,骂接骂!",
//    "squaresHateCircles"   : "小方吐口水吐到小圆脸上"                                  ,
//    "circlesHateSquares"   : "小圆吐口水吐到小方身上"                                  ,
//    "areTheyYelling"       : "(對！去拍下他們朝別人 *嘶吼* 的樣子)"              , // not used
//    "nerdsNow"             : "現在，這些書呆子們在幹麻？"                        ,
//    "schockedPeep"         : "是什麼使這位民眾感到驚嚇?"                              ,
//    "whatever"             : "管啥子"                                              ,
    // Act 3
//    "ellipsis"             : "......",
//    "coolNoMore"           : "這些帽子已經不酷了, 你沒聽說嗎？"           ,
//    "beScared"             : "杀吧!快杀吧!我老爱看这玩意了!"                                  ,
    // MANIFESTO
//    "manifesto" : [
       //"as if you viewers want GOOD news"       ,
//       "誰會想看 *這些人的互動* ?",
//       "和平只帶來無趣. 暴力則有助散播."    ,
       //"peace is boring. conflict gets clicks." ,
//       "每個故事都需要一點衝突, 所以..."  ,
       //"...GIVE THE AUDIENCE WHAT THEY WANT."   ,
//       "給我拿出觀眾們想看的！"
//    ] ,
    // -- Strings currently in graphics resources --
    // (bg_preload.png) Preload screen
//    "playingTime"        : "游玩大概5分钟",
//    "warning"            : "警告嘞:这款游戏的梗体中文和内容有\n血腥暴力,少量脏话和不适合你的玩意,\n玩这游戏的请你决定\n原版游戏来自Nicky Case制作,梗体中文来自不会取名的屑制作",
    // (cam-instructions.png) Camera Instructions
//    "pointAndClick"      : "對準 & 點擊",
    // (chyron3.png)
//    "chyronNothing"      : "哇嗚, 你的鏡頭下 *什麼東西都沒有*" ,
    // (credits0001.png)
//    "createdBy"          : "游戏原作来自",
//    "NickyCase"          : "Nicky Case",
    // (credits0002.png)
//    "manyThanks"         : "對許多遊戲測試者的感謝:",
    // (credits000[3,4,5,6].png)
//    "patreonSupporters"  : "一些赞助者:",
    // (credits0007.png)
//    "lastButNotLeast"    : "最后,我想跟你说一句好话,",
    // (credits0008.png)
//    "thankYouForPlaying" : "谢谢你的游玩",
    // (preload_play.png)
//    "playButton"         : "赶紧玩罢" ,
    // (end_button.png) Post Credits
//    "otherWorkButton"    : "原作者的其他作品" ,
//    "buyCoffeeButton"    : "贊助作者一杯咖啡" ,
//    "replayButton"       : "重新體驗遊戲" ,
    // (logo.png) Post Credits
//    "logoWBWWB"          : "分享这玩意:" ,
    // (end_prototype.png)
//    "endOfPrototype"     : "你现在已经玩没了" ,
//    "toBeContinued"      : "(没后头了,你滚吧)" ,
    // (quote0002.png)
//    "WBWWB"              : "我們的所見會塑造我們." ,
//    "WSOTATOTSU"         : "我們塑造了我們的工具，然後工具再塑造我們." ,
    // (quote0003.png)
//    "MarshallMcLuhan"    : "Marshall McLuhan" ,
    // (quote0004.png)
//    "misatrributed"      : "(我自己乱瞎编的)" ,
//};
//这里处于纪念原因,想保留旧版,就当彩蛋看吧,如果你用某些浏览器的电脑版的话,你可以右键查看源代码
